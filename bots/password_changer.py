...
def load_bot_settings():
    ...
    try:
        ...
        return data.get("password_changer", {})
    except Exception:
        return {}
...
def processed_email(email_data):
    try:
        ...
        delete_query = """
        DELETE FROM processing_password_changes
...
def get_new_profile_data():
    try:
        ...
        try:
            cursor.execute(
                "SELECT email, pass,recovery FROM processing_password_changes WHERE server_ip = %s AND bot_type = %s LIMIT 1",
                (SERVER_IP, BOT_TYPE),
            )
            row = cursor.fetchone()
            if row:
        ...
        if not row:
            cursor.execute(
                "SELECT email, pass,recovery FROM password_changer_accounts LIMIT 1"
            )
            row = cursor.fetchone()
        if not row:
            conn.close()
        ...
        if not is_existing:
            cursor.execute(
                "INSERT INTO processing_password_changes (email, pass, recovery, server_ip, bot_type, date_time) VALUES (%s, %s, %s, %s, %s, %s)",
                (email, password, recovery, SERVER_IP, BOT_TYPE, datetime.now()),
            )
            cursor.execute(
                "DELETE FROM password_changer_accounts WHERE email = %s AND pass = %s",
                (email, password),
            )
        conn.commit()
        conn.close()
