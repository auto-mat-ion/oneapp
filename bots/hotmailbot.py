...
def _send_telegram_message():
    ...
    if not bot_token:
        ...
        return
    print("Sending VPN alert to Telegram...")
...
def _watch_vpn_connection_status():
    ...
    if VPN_CONNECTION_STATUS == "connecting":
        print("VPN connection is still connecting. Sending alert...")
...
def click_join_family_link_btn(driver, new_profile_data):
    try:
        ...
        try:
            ...
            time.sleep(2)
            while click_looks_good_button(driver):
                time.sleep(3)
                print(f"{email} : Clicked looks good button")
            # time.sleep(3)
