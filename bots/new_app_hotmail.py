...
BOT_TYPE = "second_app_hotmail"


def execute_db_action(action, retries=5, delay=3):
    attempt = 1
    while attempt <= retries:
        try:
            ...
            if attempt == retries:
                raise
            print(
                f"Database operation failed (attempt {attempt}/{retries}): {exc}. Retrying in {delay} seconds..."
            )
            time.sleep(delay)
            attempt += 1
...
def get_db_connection():
    try:
        ...
        print(f"Unable to connect to database after retries: {exc}")
        return None
...
def connect_new_random(country=None):
    try:
        ...
        # Find current connected country and disconnect if its the same as preferred country to get new ip from same country
        current_location = get_current_location().lower()
        preferred = "usa" if country.lower() in ["usa", "united states"] else country
        if preferred.lower() in current_location:
            print(f"EXPRESS: Currently connected to {current_location}. Continuing...")
            return True
...
def click_personal_acc_button(driver):
    """
    Clicks the next button
    """
    try:
        NEXT_BUTTON_ELEMENT = (By.CSS_SELECTOR, 'div[id="msaTile"]')
...
def is_your_account_has_been_locked_page(driver):
    ...
    try:
        ...
        if (
            ("account recovery has been blocked" in title_element.text.lower())
            or ("something went wrong" in title_element.text.lower())
            or ("let's prove you're human" in title_element.text.lower())
        ):
            return True
        else:
            return False
...
def processed_email(email_data):
    try:
        ...
        def db_action():
            ...
            insert_query = """
            INSERT INTO second_app_processed_emails (server_ip, bot_type,date_time, email, password, recovery, country)
            ...
            delete_query = """
            DELETE FROM second_app_processing_accounts
    ...
    with lock:
        try:
            ...
            def db_action():
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "INSERT INTO second_app_signin_log (server_ip, bot_type, email, log_time, status, error) VALUES (%s, %s, %s, %s, %s, %s)",
...
def update_accounts_data(
    ...
    try:
        ...
        def db_action():
            ...
            cursor.execute(
                "SELECT account_id FROM second_app_accounts_details WHERE email = %s",
                (email,),
            )
            result = cursor.fetchone()
            ...
            if result:
                ...
                if update_fields:
                    ...
                    query = f"UPDATE second_app_accounts_details SET {', '.join(update_fields)} WHERE email = %s"
                    cursor.execute(query, tuple(update_values))
            else:
                cursor.execute(
                    "INSERT INTO second_app_accounts_details (server_ip, bot_type, date_time, email, password, recovery,country,save_smtp) VALUES (%s, %s, %s, %s, %s,%s,%s, %s)",
                    (
                        SERVER_IP,
                        BOT_TYPE,
                        datetime.now(),
...
def all_done_page(driver):
    ...
    try:
        ...
        if success_element.text.lower().startswith("you're now signed in"):
            return True
        else:
            return False
    except:
...
def load_cache():
    try:
        ...
        result = execute_db_action(db_action)
...
def failed_smtp(email_address, password, temp_email):
    try:
        ...
        def db_action():
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO second_app_failed_smtp (server_ip, bot_type, date_time, email, password, recovery) VALUES (%s, %s, %s, %s, %s, %s)",
                (
                    SERVER_IP,
                    BOT_TYPE,
                    datetime.now(),
...
def initialize_new_profile(new_profile_data):
    ...
    try:
        ...
        password = new_profile_data.get("pass")
        recovery = new_profile_data.get("recovery")
        country = new_profile_data.get("country")

        new_profile_data_original = new_profile_data.copy()

        vpn_country = "poland" if country.lower() == "poland2" else country
        ...
        click_personal_acc_button(driver)
        ...
        if not enter_recovery_email_2(driver=driver, recovery_email=recovery):
            ...
            return False, "Error entering recovery email", driver
        time.sleep(0.5)
        bring_to_front(driver)
        time.sleep(1)
        ...
        if status:
            ...
            failed_smtp(email_address, password, recovery)
            return False, f"Error setting up SMTP: {error}", driver
...
def get_new_profile_data():
    try:
        ...
        def db_action():
            ...
            try:
                cursor.execute(
                    "SELECT email, pass,recovery, country FROM second_app_processing_accounts WHERE server_ip = %s AND bot_type = %s LIMIT 1",
                    (SERVER_IP, BOT_TYPE),
                )
                row = cursor.fetchone()
                if row:
            ...
            if not row:
                cursor.execute(
                    "SELECT email, pass,recovery, country FROM second_app_input_accounts LIMIT 1"
                )
                row = cursor.fetchone()
            if not row:
                conn.close()
                return False, {"email": "", "pass": "", "recovery": "", "country": ""}

            email, password, recovery, country = row
            if not is_existing:
                cursor.execute(
                    "INSERT INTO second_app_processing_accounts (email, pass, recovery, server_ip, bot_type, date_time) VALUES (%s, %s, %s, %s, %s, %s)",
                    (email, password, recovery, SERVER_IP, BOT_TYPE, datetime.now()),
                )
                cursor.execute(
                    "DELETE FROM second_app_input_accounts WHERE email = %s AND pass = %s",
                    (email, password),
                )
            conn.commit()
            conn.close()
